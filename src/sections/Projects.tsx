...
const ProjectCard = ({ project, index, layout, onClick }: ProjectCardProps) => {
  ...
  return (
    <motion.div
      ...
      className={`group relative flex flex-col overflow-hidden rounded-2xl border border-charcoal/5 hover:border-charcoal/20 hover:shadow-lg transition-all duration-500 cursor-none bg-surface/70 ${wrapperClasses[layout]} max-md:!col-span-1 max-md:!min-h-0`}
      onClick={onClick}
      role="button"
      tabIndex={0}
      onKeyDown={(e) => {
    ...
    >
      ...
      <div className={`flex flex-col ${innerLayout[layout]} h-full max-md:!flex-col`}>
        ...
        >
          ...
          <div className="absolute top-3 right-3 md:top-4 md:right-4 z-10 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-2 -translate-y-2 group-hover:translate-x-0 group-hover:translate-y-0">
        ...
        <div className={`flex flex-col justify-between p-3 md:p-5 lg:p-6 ${contentSize[layout]} max-md:!w-full ${isFeatured ? 'md:py-8 md:px-8' : ''}`}>
          <div className="flex flex-col space-y-1.5 md:space-y-3">
            ...
            <div className="hidden md:flex items-center space-x-2 font-hud text-[9px] text-sage tracking-widest">
